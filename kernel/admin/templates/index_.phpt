...
<link href="/admin/textbox.css" type="text/css" rel="stylesheet" >
...
<?foreach($args['modules'] as $i)if($i['section']=='modules'){?>
			<li class="<?=($i['current']||$i['name']==current(explode("_",$_GET['mod']))? 'selected' : '')?>"><a href="/admin/?mod=<?=htmlspecialchars($i['name'])?>" title="<?=htmlspecialchars($i['descr'])?>"><?=htmlspecialchars($i['title'])?></a></li>
<?}?>
			</ul>
			</div>
...
<?}?>
	...
	<div class="path"><img src="/admin/images/bullet3.gif" width="9" height="7" alt="" >&nbsp;&nbsp;<a href="/admin/">CMS</a><?=(sizeof($args['submod'])>0)?' / <a href="/admin/?mod='.$args['submod']['name'].'">'.$args['submod']['title'].'</a>':''?> / <a href="/admin/?mod=<?=htmlspecialchars($args['mod'])?>"><?=htmlspecialchars($args['title'])?></a><?=($args['subtitle']? ' / '.htmlspecialchars($args['subtitle']) : '')?><?=($args['fullpath']? ' / '.($args['fullpath']) : '')?></div>
	</td>
<?if($args['help']){?>
