...
<form action="?mod=<?=$_GET['mod']?><?=isset($_GET['order']) && $_GET['order'] ? '&order='.$_GET['order'].'&type='.$_GET['type']:''?><?=isset($_GET['page']) && $_GET['page'] ? '&page='.$_GET['page']:''?>&act=edititem&id=<?=$_GET['id']?>" method="post" enctype="multipart/form-data">
