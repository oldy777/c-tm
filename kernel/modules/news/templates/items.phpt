...
function del(id)
{
  ...
  {
    document.location = '/admin/?mod=<?=$_GET['mod']?>&act=delitem&id='+id+'<?=isset($_GET['order']) && $_GET['order'] ? '&order='.$_GET['order'].'&type='.$_GET['type']:''?><?=isset($_GET['page']) && $_GET['page'] ? '&page='.$_GET['page']:''?>';
  }
}
</script>
<?if($args['mod_pos']){?>
  ...
  <?if($f['view']==1){?>
    <td align="left" id="<?=$i['id']?>">
        <?if(isset($f['link']) && $f['link']==1){?>
             <a href="?mod=<?=$_GET['mod']?><?=isset($_GET['order']) && $_GET['order'] ? '&order='.$_GET['order'].'&type='.$_GET['type']:''?><?=isset($_GET['page']) && $_GET['page'] ? '&page='.$_GET['page']:''?>&act=edititem&id=<?=$i['id']?>" title="редактировать">
...
<?}?>
    <td nowrap="nowrap">
      <a href="?mod=<?=$_GET['mod']?><?=isset($_GET['order']) && $_GET['order'] ? '&order='.$_GET['order'].'&type='.$_GET['type']:''?><?=isset($_GET['page']) && $_GET['page'] ? '&page='.$_GET['page']:''?>&act=edititem&id=<?=$i['id']?>" title="редактировать"><img class="btn_edit" src="/admin/images/icon_edit.gif" width="17" height="17" border="0" /></a>
...
<?if  ($args['pages'] > 1) { ?>
    ...
    <? } ?>
        ...
        <? if ($args['page'] != 'all') { ?>
        <li><a class="pages-right" href="/admin/?mod=<?=$_GET['mod']?><?=isset($_GET['order']) && $_GET['order'] ? '&order='.$_GET['order'].'&type='.$_GET['type']:''?>&page=all">Все</a></li>
        <?}else{?>
