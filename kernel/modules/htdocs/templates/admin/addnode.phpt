...
<?if($errors['path']['empty']){?>&bull; поле &laquo;<a href="#path">Имя папки</a>&raquo; пусто;<br /><?}?>
<?if($errors['path']['valid']){?>&bull; поле &laquo;<a href="#path">Имя папки</a>&raquo; может содержать только символы латинского алфавита, цифры, "_", "-";<br /><?}?>
<?if($errors['path']['unique']){?>&bull; поле &laquo;<a href="#path">Имя папки</a>&raquo; не уникально, раздел с таким именем уже существует;<br /><?}?>
<?if($errors['title']){?>&bull; поле &laquo;<a href="#title">Название раздела</a>&raquo; пусто;<br /><?}?>
</div>
<?}?>
<form method="post" action="?mod=htdocs&amp;act=addnode&amp;id=<?=$args['parent']['id']?>" onsubmit="return checkform(this) && submitonce(this)" enctype="multipart/form-data">
<fieldset>
<legend>
...
<table id="panelmain" width="100%" border="0" cellspacing="2" cellpadding="2" class="table" style="border:none">
  ...
  <tr>
    <th align="right" class="<?=($errors['path']? 'error' : '')?>" nowrap="nowrap">* Имя папки:</th>
    <td nowrap="nowrap">
      ...
      <td width="99%" class="none"><input type="text" class="none" name="path" id="path" style="width:100%" value="<?=trim($args['path'])?>" /></td>
      </tr></table>
    </td>
  </tr>
  <tr>
    <th align="right" class="<?=($errors['title']? 'error' : '')?>" nowrap="nowrap">* Название раздела:</th>
...
<table id="panelext" width="100%" border="0" cellspacing="2" cellpadding="2" class="table" style="border:none" id="">
  <colgroup>
    <col width="25%" />
    <col width="75%" />
  </colgroup>
  <tr>
    ...
    <td>
      <select name="id_maket">
        ...
        <option value="<?=$args['parent']['id_maket']?>" selected>наследовать (id:<?=$args['parent']['id_maket']?>)</option>
...
<?}?>
  ...
  <tr>
    ...
    <td>
      ...
      <input type="checkbox" class="checkbox" name="keywords" id="keywordsNULL" value="NULL"<?=($args['keywords']===NULL? ' checked="checked"' : '')?> /><label for="keywordsNULL">наследовать от родителя</label>
    </td>
  </tr>
  <tr>
    <th align="right" nowrap="nowrap">
    ...
    <td>
      <textarea name="description" style="width:100%;height:40x"><?=trim($args['description'])?></textarea><br />
      <input type="checkbox" class="checkbox" name="description" id="descriptionNULL" value="NULL"<?=($args['description']===NULL? ' checked="checked"' : '')?> /><label for="descriptionNULL">наследовать от родителя</label>
    </td>
  </tr>
</table>
</fieldset>
<table width="100%" border="0" cellspacing="2" cellpadding="4">
  <tr>
   <td width="25%">&nbsp;</td>
   <td width="75%">
     <input type="submit" class="button" value="Добавить папку!" />
     <input type="submit" class="button" value="Далее" name="next" />
